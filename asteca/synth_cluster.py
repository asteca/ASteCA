...
def randVals(theor_tracks, st_dist_mass):
    ...
    function.
    """
    print("Generating lists of random values")
...
def synthcl_generate(
    ...
    * M_total_flag = True
    ...
    if not isoch_cut.any():
        # if M_total_flag:
        #     return synth_clust, N_ev, N_phot
        return synth_clust

    # Interpolate IMF's sampled masses into the isochrone.
    isoch_mass = mass_interp.main(
        isoch_cut, m_ini_idx, synthcl_dict["st_dist_mass"][ml])
    ...
    isoch_binar = binarity.main(
        self.binarity, isoch_mass, alpha, beta, m_ini_idx,
        synthcl_dict["rand_unif_vals"][1]
    )
    ...
    isoch_compl = isoch_compl[:, : cluster_dict["N_obs_stars"]]

    # Assign errors according to errors distribution.
    synth_clust = add_errors.main(
        isoch_compl, cluster_dict["err_lst"], synthcl_dict["rand_norm_vals"][1]
    )

    # if M_total_flag:
    #     # mass_cumsum = synthcl_dict["st_dist_mass"][ml][1]

    #     # # Filter the total according to the mass range in isoch_cut
    #     # M_total_arr = mass_cumsum[msk_m]

    #     # # Apply 'msk_ct' and 'msk_cr' to correct the mass for the stars removed
    #     # # by extinction() and completeness_rm() (if applied)
    #     # if msk_cr.any():
    #     #     M_total = M_total_arr[msk_ct][msk_cr][: cluster_dict["N_obs_stars"]][-1]
    #     # else:
    #     #     M_total = M_total_arr[msk_ct][: cluster_dict["N_obs_stars"]][-1]
    ...
    #     N_phot = int(np.array([N_phot1 + N_phot2 + N_phot3])*N_ratio)
    #     N_ev = int(N_ev*N_ratio)
...
def properModel(met_age_dict, model_fixed, model_fit):
    ...
    model_full = np.array(
        [model_comb[k] for k in ["z", "loga", "alpha", "beta", "Av", "DR", "Rv", "dm"]]
    )
