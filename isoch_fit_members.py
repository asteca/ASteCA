...
# Store paths and names to all clusters processed by the 'cluster analysis'
# algorithm.
file_path = join(dir_memb_files, 'algor_analisys_'+algor_sr+'.data')                   
data_temp = np.genfromtxt(file_path, dtype=None)
data_temp = zip(*data_temp)
dir_files = [[], []]
for item in data_temp[0]:
    dir_files[0].append(item[:-14])
    dir_files[1].append(item[-13:])
...
for f_indx, sub_dir in enumerate(dir_files[0]):
    ...
    clust_name = dir_files[1][f_indx]
    
    tik = time.time()
    print '\n%s/%s' % (str(sub_dir), str(clust_name))
    ...
    # Check if file is not empty. 38 bytes is the length of a file with only
    # the first line written on it.
    if os.path.getsize(file_path) > 38:
