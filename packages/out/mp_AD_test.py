...
def pl_ad_test(gs, plot_style, flag_ad_test, ad_cl, ad_fr, id_kinem):
    """
    """
    if flag_ad_test:
...
def pl_p_vals(
    ax, plot_style, Ncl, Nf, prob_cl, kde_cl, kde_fr, x_cl, x_fr, x_over,
        y_over, reg_id):
    """
    """
    ax.set_title(r'$P_{{cl}}={:.2f}\;({})$'.format(prob_cl, reg_id))
    ax.axes.yaxis.set_ticklabels([])
    plt.xlabel('p-values')
    ...
    leg = ax.legend(handles, labels, numpoints=1, )
    leg.get_frame().set_alpha(0.6)
    plt.gca().set_ylim(bottom=0)


def pl_ad_pvals_phot(gs, plot_style, flag_ad_test, ad_cl_fr_p):
    if flag_ad_test:
        Ncl, Nf, prob_cl, kde_cl, kde_fr, x_cl, x_fr, x_over, y_over =\
            ad_cl_fr_p
        ax = plt.subplot(gs[0:2, 2:4])
        pl_p_vals(
            ax, plot_style, Ncl, Nf, prob_cl, kde_cl, kde_fr, x_cl, x_fr,
            x_over, y_over, 'phot')


def pl_ad_pvals_pk(gs, plot_style, flag_ad_test, ad_cl_fr_pk, id_kinem):
    # Only plot if either parallax or PMs or radial velocities are defined
    pd_Plx, pd_PMRA, pd_RV = id_kinem[0], id_kinem[2], id_kinem[6]
    # Define first row depending on whether kinematic data was defined.
    k_flag = np.array([_ != 'n' for _ in (pd_Plx, pd_PMRA, pd_RV)]).any()
