...
def main(isochrone, e, d, Rv, ext_coefs):
    '''
    ...
    for mi, mag in enumerate(isochrone[0]):

        # mx = Mx + dist_mod + Ax
        # Ax = cx * Av
        #
        Ax = ext_coefs[0][mi] * Av
        iso_moved[0].append(np.array(mag) + d + Ax)

    # Loop through colors.
    for ci, col in enumerate(isochrone[1]):

        # (C1 - C2) = (C1 - C2)o + A1 - A2; A1 - A2 = Av * (c1 - c2)
        # Ax = (c1 - c2) * Av = c * Av
        #
        Ax = ext_coefs[1][ci] * Av
        iso_moved[1].append(np.array(col) + Ax)
