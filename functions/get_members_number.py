...
def get_memb_num(backg_value, clust_rad, stars_in, stars_in_rjct):
    ...
    the total number of stars inside the radius, n_t = n_c + N_f, and then
    ...
    # If value is negative or zero, assign 1/10th of the total number of stars
    # inside the cluster's radius or 9 stars, whichever is larger.
    flag_no_memb = False
    if n_c <= 0:
        n_c = max(int(n_t/10), 9)
        flag_no_memb = True
        
    # Raise a flag if the number of members is <10
    flag_num_memb_low = False if n_c >= 10 else True
    
    return int(n_c), flag_num_memb_low, flag_no_memb
