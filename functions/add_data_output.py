...
def add_data_output(out_file_name, sub_dir, output_dir, clust_name,
    ...
    line = [str(sub_dir) + '/' + str(clust_name),
        str('%.2f' % round(center_cl[0])), str('%.2f' % round(center_cl[1])),
        str('%.2f' % round(clust_rad)), str('%.2f' % round(k_prof[0])),
        str('%.2f' % round(np.sqrt(k_pr_err[0][0]))),
        str('%.2f' % round(k_prof[1])),
        str('%.2f' % round(np.sqrt(k_pr_err[1][1]))),
